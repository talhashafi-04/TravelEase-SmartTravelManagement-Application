...
FROM PAYMENT";
            ...
            if (dtSuccessFailure.Rows.Count > 0)
            {
                ...
                int success = row.IsNull("SuccessCount")
                    ? 0
                    : Convert.ToInt32(row["SuccessCount"]);
                int failure = row.IsNull("FailureCount")
                    ? 0
                    : Convert.ToInt32(row["FailureCount"]);
                double total = success + failure;
                if (total > 0)
                {
                    series.Points.AddXY("Success", success / total);
                    series.Points.AddXY("Failure", failure / total);
                    // Display percentages outside the pie
        ...
        private void LoadChargebackRateChart()
        {
            ...
            var series = new Series("ChargebackRate")
            {
                ChartType = SeriesChartType.Column,
                IsValueShownAsLabel = true,
                Label = "#VALY{P0}"  // show values as percentages
            };
...
SELECT
  ...
  CAST(
    SUM(CASE WHEN FailureReason = 'Chargeback' THEN 1 ELSE 0 END) * 1.0 
    / NULLIF(COUNT(*), 0) 
    AS DECIMAL(5,2)
  ) AS Rate
FROM PAYMENT
            ...
            using (var dlg = new FolderBrowserDialog { Description = "Select folder to save CSV reports" })
            {
                ...
                try
                {
                    WriteDataTableToCsv(dtSuccessFailure,
                        Path.Combine(folder, "PaymentSuccessFailureRate.csv"));
                    WriteDataTableToCsv(dtChargeback,
                        Path.Combine(folder, "MonthlyChargebackRate.csv"));
